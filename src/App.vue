...
const ItemsForList = computed(() => {
  ...
  switch (selectState.ByPriceId) {
    case 1:
      list = list.sort((i, n) => i.price.current_price - n.price.current_price);
      break;
    case 2:
      list = list.sort((i, n) => n.price.current_price - i.price.current_price);
      break;
    default:
      break;
  }
...
const changeLocalStorage = (option) => {
  if (Storage.value[option.key].includes(option.id)) {
    Storage.value[option.key] = Storage.value[option.key].filter(
      (el) => el !== option.id
    );
  } else {
...
<template>
  <main class="wrapper">
    <a href=""></a>
    <h3 class="linkString">
      <a href="#">Главная</a>/<a href="#">Системы хранения</a>/<a
        href="#"
        class="linkString-active"
